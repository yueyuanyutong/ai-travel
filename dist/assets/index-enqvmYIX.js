...
}`},{role:"user",content:e}],temperature:.7,max_tokens:2e3,stream:!1})})).json();if(console.log("API Response:",r),r.error)throw console.error("API Error:",r.error),new Error(r.error.message||"调用AI服务失败");try{const s=r.choices[0].message.content.trim();return console.log("AI Response Content:",s),JSON.parse(s)}catch(s){return console.error("解析AI响应失败:",s),this.parseAIResponse(r.choices[0].message.content)}}catch(t){throw console.error("解析用户输入失败:",t),new Error("无法解析您的需求，请重试")}}async generateItinerary(e){try{const r=await(await fetch(`${this.baseUrl}/chat/completions`,{method:"POST",headers:{"Content-Type":"application/json",Authorization:`Bearer ${this.apiKey}`},body:JSON.stringify({model:"deepseek-chat",messages:[{role:"system",content:`你是一个旅行规划专家。请根据提供的参数生成详细的旅行计划。请严格按照以下JSON格式返回：
{
  ...
  `}const D={"/":"home","/plan":"plan","/booking":"booking","/profile":"profile","/login":"login","/register":"register"};class J{constructor(){this.currentPage="home",this.init()}init(){window.addEventListener("popstate",()=>{this.handleRoute(window.location.pathname)}),this.handleRoute(window.location.pathname)}handleRoute(e){const t=D[e]||"home";this.currentPage=t,this.renderPage(t)}navigate(e){window.history.pushState({},"",e),this.handleRoute(e)}renderPage(e){const t=document.getElementById("app");e!=="login"&&e!=="register"&&(t.innerHTML=L());const r=document.createElement("div");switch(t.appendChild(r),e){case"home":this.renderHome(r);break;case"plan":this.renderPlan(r);break;case"booking":this.renderBooking(r);break;case"profile":this.renderProfile(r);break;case"login":this.renderLogin(t);break;case"register":this.renderRegister(t);break}}renderHome(e){e.innerHTML=`
        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <div class="space-y-6">
            ...
            <div class="card">
              ...
              <div class="space-y-2">
                <button class="w-full btn-primary mb-2" onclick="window.print()">
                  打印行程
                </button>
                <button class="w-full btn-secondary" onclick="localStorage.removeItem('currentItinerary'); router.navigate('/')">
                  清除行程
                </button>
              </div>
            </div>
